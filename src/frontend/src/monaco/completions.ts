...
type EntryKind = 'keyword' | 'class' | 'function' | 'module' | 'method' | 'constant'
...
function scanCpp(model: monaco.editor.ITextModel): Map<string, string> {
  ...
  const aliasRe =
    /\busing\s+([A-Za-z_]\w*)\s*=\s*((?:std::)?[A-Za-z_]\w*(?:::\w+)*)\s*(?:<[^;{}()]*>)?\s*;/g
...
function scanJava(model: monaco.editor.ITextModel): Map<string, string> {
  ...
  const map = new Map<string, string>()
  const modifiers =
    '(?:private |protected |public |static |final |volatile |transient |synchronized )*'
  for (const type of Object.keys(METHOD_TABLES.java)) {
    const re = new RegExp(
      `\\b${modifiers}(?:[A-Za-z_][\\w.]*)?\\s*${type}\\s*(?:<[^;{}()]*>)?\\s+([A-Za-z_]\\w*)\\s*(?:=|;)`,
      'g',
    )
    let m: RegExpExecArray | null
...
export function registerProblemCompletions(): void {
  ...
  disposables.push(
    ...
    monaco.languages.registerCompletionItemProvider(
      'python',
      makeProvider('python', pythonCompletions),
    ),

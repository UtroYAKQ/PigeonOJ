export default {
  config: {
    key: 'Key',
    value: 'Value',
    description: 'Description',
    updater: 'Updated by',
    updatedAt: 'Updated at',
    empty: 'No settings in this category',
    saveSuccess: 'Configuration saved',
    loadFailed: 'Failed to load configuration',
    saveFailed: 'Failed to save configuration',
    editTitle: 'Edit configuration: {key}',
    booleanOn: 'Enabled',
    booleanOff: 'Disabled',
    manualReview: 'Manual review',
    autoApprove: 'Auto approve',
    smtpMode: {
      ssl: 'SSL (implicit TLS, port 465)',
      starttls: 'STARTTLS (upgrade after plaintext, port 587)',
      plain: 'Plaintext (intranet only, port 25)',
    },
    htmlTemplateLabel: 'HTML template',
    htmlTemplateHint: 'Placeholders: {code} = code, {purpose} = purpose label (e.g. "register")',
    preview: 'Email code card preview',
    previewPurpose: 'Register',
    uploadLogo: 'Upload image',
    logoPlaceholder: 'Enter an external image URL, or leave empty for the default icon',
    logoHint: 'Upload a local image (JPG / PNG / WEBP / GIF, ≤5MB) or enter an external URL',
    jsonInvalid: 'Invalid JSON — fix it before saving',
    itemsCount: '{count} item(s)',
    announcementLabel: 'Text announcement',
    uploadImage: 'Upload',
    changeImage: 'Change',
    bannerTitle: 'Title (optional, shown on the banner)',
    bannerLink: 'Link (optional, e.g. /contests)',
    addBanner: 'Add banner',
    removeBanner: 'Remove',
    bannersMax: 'Up to 10 banners',
    bannersEmpty: 'No banners yet — click "Add banner" and upload an image',
    homeBanners: 'Carousel banners',
    homeBannersHint: 'Shown in order on the home carousel; click a thumbnail to upload, up to 10',
    homeAnnouncement: 'Site announcement',
    homeAnnouncementHint: 'Shown in the home announcement card, Markdown supported; saving empty content clears it',
    announcementPlaceholder:
      'Markdown supported: headings, lists, links, $math$… Save empty to clear',
  },
  admin: {
    backToApp: 'Back to site',
    users: {
      title: 'Users',
      search: 'Search nickname / email',
      avatar: 'Avatar',
      user: 'User',
      email: 'Email',
      role: 'Role',
      status: 'Status',
      lastLogin: 'Last login',
      registered: 'Registered',
      empty: 'No matching users',
      rolesUpdated: 'Roles updated',
      pickRole: 'Select a role first',
      roleTitle: 'Set roles: {name}',
      reasonPrompt: 'Enter a reason for {action} (optional):',
      reasonTooLong: 'Reason is too long',
      banReason: 'Violation / anomaly reason',
      freezeReason: 'Freeze reason',
      freezeUntil: 'Freeze until',
      freezeHint:
        'Freeze is a short-term ban that auto-expires; the end time must be in the future and within 7 days.',
      freezeUntilInvalid: 'Invalid freeze end time: must be in the future and within 7 days',
      ban: 'Ban',
      unban: 'Unban',
      freeze: 'Freeze',
      unfreeze: 'Unfreeze',
      confirmUnban: 'Unban user "{name}"?',
      confirmUnfreeze: 'Unfreeze user "{name}"?',
    },
    sandbox: {
      title: 'Sandbox node status',
      online: 'Online {online} / {total}',
      avgLoad: 'Average online-node load {load}%',
      load: 'Load',
      memory: 'Memory',
      tasks: 'Running tasks',
      version: 'Sandbox version',
      empty: 'No sandbox nodes',
    },
    online: {
      summary: '{count} online now',
      avatar: 'Avatar',
      user: 'User',
      device: 'Device',
      ip: 'IP / Location',
      empty: 'No online users',
    },
    submissions: {
      problem: 'Problem',
      submitter: 'Submitter',
      searchUser: 'Search submitter nickname',
      problemQuery: 'Problem ID',
      problemNotFound: 'No problem matches this ID',
      empty: 'No matching submissions',
    },
    reports: {
      title: 'Reports',
      type: 'Type',
      target: 'Target',
      reporter: 'Reporter',
      reason: 'Reason',
      status: 'Status',
      createdAt: 'Created at',
      handledAt: 'Handled at',
      empty: 'No reports',
      handled: 'Handled',
      handleSuccess: 'Report handled',
      ignored: 'Ignored',
      handleTitle: 'Handle report: {target}',
      approve: 'Approve & remove content',
      ignore: 'Ignore report',
    },
    tags: {
      title: 'Tag management',
      name: 'Name',
      color: 'Color',
      status: 'Status',
      createdAt: 'Created at',
      active: 'Active',
      archived: 'Archived',
      create: 'New tag',
      edit: 'Edit tag',
      archive: 'Archive',
      namePlaceholder: 'Tag name',
      search: 'Search tags',
      archiveConfirm:
        'Archived tags can no longer be selected; existing problem links are kept. Archive "{name}"?',
      created: 'Tag created',
      updated: 'Tag updated',
      archivedSuccess: 'Tag archived',
      empty: 'No tags yet',
    },
    teams: {
      search: 'Search by team name',
      avatar: 'Avatar',
      team: 'Team',
      creator: 'Creator',
      memberCount: 'Members',
      status: 'Status',
      createdAt: 'Created at',
      empty: 'No teams match the filters',
      totalCount: '{count} teams in total',
      create: 'Create team',
      createSuccess: 'Team created',
      detailTitle: 'Team detail',
      loadFailed: 'Failed to load team',
      backToList: 'Back to teams',
      overviewTitle: 'Team info',
      description: 'Description',
      descEmpty: 'This team has no description yet',
      teamId: 'Team ID',
      disbandedAt: 'Disbanded at',
      statusActive: 'Active',
      statusDisbanded: 'Disbanded',
      tabMembers: 'Members',
      tabProblems: 'Team problems',
      tabSets: 'Team problem sets',
      tabContests: 'Team contests',
      statProblems: 'Problems',
      statSets: 'Problem sets',
      statContests: 'Contests',
      memberJoinedAt: 'Joined at',
      membersEmpty: 'No members',
      problemSearch: 'Search team problems',
      problemsEmpty: 'No problems in the team bank yet',
      problemLimits: 'Limits',
      problemSubmissions: 'Submissions / Accepted',
      setVisible: 'Problems',
      setCreated: 'Created at',
      setsEmpty: 'No team problem sets yet',
      contestRule: 'Rule',
      contestTime: 'Time frame',
      contestsEmpty: 'No team contests yet',
      referenced: 'Referenced',
      created: 'Created',
    },
    logs: {
      title: 'Logs',
      request: 'Request logs',
      login: 'Login logs',
      exception: 'Exception logs',
      noExport: 'No data on this page to export',
      requestSearch: 'Search request ID / path',
      loginSearch: 'Search email / action',
      exceptionSearch: 'Search message / stack',
      nicknameSearch: 'Search by username',
      start: 'Start time',
      end: 'End time',
      requestId: 'Request ID',
      userId: 'User ID',
      nickname: 'User',
      method: 'Method',
      path: 'Path',
      code: 'Status code',
      duration: 'Duration (ms)',
      location: 'Location',
      device: 'Device',
      time: 'Time',
      action: 'Action',
      result: 'Result',
      reason: 'Reason',
      level: 'Level',
      message: 'Message',
      requestEmpty: 'No request logs',
      loginEmpty: 'No login logs',
      exceptionEmpty: 'No exception logs',
      failed: 'Failed',
      viewTraceback: 'Stack',
      tracebackTitle: 'Exception detail',
      noTraceback: 'No stack trace for this exception',
      clearTitle: 'Clear {type}',
      clearConfirm: 'All logs of this type will be permanently deleted. Continue?',
      clearAction: 'Clear logs',
      cleared: 'Logs cleared',
    },
  },
  dictionary: {
    reportStatus: { pending: 'Pending', handled: 'Handled', ignored: 'Ignored' },
    reportType: {
      problem: 'Problem',
      solution: 'Solution',
      post: 'Post',
      comment: 'Comment',
      user: 'User',
    },
    logLevel: { error: 'Error', warning: 'Warning', fatal: 'Fatal' },
    sandboxStatus: { online: 'Online', offline: 'Offline' },
    category: {
      site: 'Site',
      auth_email: 'Authentication / email',
      team: 'Teams',
      contest: 'Contests',
      sandbox: 'Sandbox',
      log: 'Logs',
      community: 'Community',
    },
  },
}

...
目标结构（对齐 vue-fastapi-admin 风格的按技术层分包）：
    app/api/v1/**       路由层（最上层）
    app/controllers/**  业务控制器（业务逻辑 + 数据访问仓储）
    app/models/**       ORM 模型
    app/schemas/**      Pydantic 契约模型
    app/core/**         核心设施（database / redis / storage / exceptions / dependency / middlewares）
    app/utils/**        纯工具（安全 / 响应信封 / 分页 / 校验）
    app/settings/**     配置

规则：
1. app.api 仅可被 api 层内部引用；其余任何层 import app.api 即违规（路由层不可被下穿依赖）
2. models / schemas 不得 import app.controllers 或 app.api（契约层不依赖业务逻辑）
3. utils 必须保持纯净：不得 import app.api / controllers / core / models / schemas
...
LAYERS = ("api", "controllers", "models", "schemas", "core", "utils", "settings", "log", "rpc_gen")
    ...
    for node in ast.walk(tree):
        ...
        for target in imported_targets(node, pkg):
            ...
            if target_layer is None or target_layer == own_layer:
                continue
            # 规则 1+2：路由层只能被自己人 import
            if target_layer == "api":
                problems.append(f"{dotted(path)}:{line}: {own_layer} 层违规依赖路由层 -> {target}")
                continue
            # 规则 3：契约层不依赖业务逻辑
            if own_layer in ("models", "schemas") and target_layer == "controllers":
                problems.append(f"{dotted(path)}:{line}: {own_layer} 违规依赖 controllers -> {target}")
                continue
            # 规则 4：utils 保持纯净
            if own_layer == "utils" and target_layer in ("controllers", "models", "schemas", "api"):
                problems.append(f"{dotted(path)}:{line}: utils 违规依赖 {target_layer} -> {target}")
    return problems

"""用户域控制器：认证（注册 / 登录 / 会话 / 找回密码）、用户中心（资料 / 注销 /
会话管理）、用户管理（角色 / 封禁 / 冻结；端点挂 /admin 前缀，由 admin 路由调用）
与数据访问仓储。原 auth 模块已并入本控制器。
"""
...
from app.controllers.audit import write_login_log
from app.core.redis import redis_delete, redis_get_json, redis_incr, redis_set, redis_set_json
...
from app.controllers.system_config import ConfigService
...
class RoleRepository:
    ...
    async def get_global_role_codes(self, user_id: uuid.UUID) -> list[str]:
        ...
        stmt = (
            ...
            .where(UserRole.user_id == user_id, UserRole.scope == "global", UserRole.object_id.is_(None))
        )
        return list((await self.db.execute(stmt)).scalars().all())
    ...
    async def replace_global_roles(self, user_id: uuid.UUID, role_ids: list[uuid.UUID]) -> None:
        ...
        for role_id in role_ids:
            self.db.add(UserRole(user_id=user_id, role_id=role_id, scope="global", object_id=None))
        await self.db.flush()

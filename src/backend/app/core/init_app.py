...
async def lifespan(_: FastAPI):
    """应用生命周期：启动判题网关 gRPC 服务、巡检循环与比赛状态推进，退出时优雅关闭。"""
    ...
    contest_task = asyncio.create_task(contest_transition_loop())
    yield
    maint_task.cancel()
    try:
        await maint_task
...
async def contest_transition_loop(interval: int = 30) -> None:
    """比赛状态推进循环：开赛 / 自动封榜 / 结束（解冻始终人工触发）。"""
    ...
    while True:
        try:
            ...
            raise
        except Exception:  # noqa: BLE001 - 单轮失败不影响后续推进
            import logging

            logging.getLogger(__name__).exception("比赛状态推进循环异常")

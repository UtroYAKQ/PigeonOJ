...
| --- | --- | --- | --- | --- | --- |
| GET | /problems | public / auth | 题库列表。默认（scope=all）题库中心仅 published+public；`scope=mine` 为管理视图（须登录）：创建者见自己全部题目，管理角色（admin/tutor/team_creator）见可管理范围全量，可叠加 `status` 过滤；mine 视图每项附 `needs_reverification`（未验题或测试点/样例晚于最近验题通过时间变更） | 分页/标签/难度/关键字/scope/status | problem[] |
...
| PUT | /problems/{id}/test-cases | admin/tutor/team_creator/team_admin | 更新样例 / 测试点（出题不设分值；提交得分由判题服务端按通过比例派生，比赛计分随 contests 模块配置） | cases[]（input、expected_output、is_sample、sort_order） | - |
| POST | /problems/{id}/verify | admin/tutor/team_creator/team_admin（发起 / 自行验题）/ 受邀验题人（凭 invite token 提交） | 发起验题 / 提交验题代码（双模式请求体：`code+language` 为提交，否则为发起）；出题人与管理角色可随时自行验题，不受 pending 记录归属限制 | verifier_id?/invite_expires_hours?/invite_token?/code?/language? | verification 或 submission_id |
| GET | /verify-invites/{token} | public | 解析验题邀请链接（返回题面与样例供受邀人查看；不含正式测试点内容与题解） | - | {problem_id, problem_title, expires_at, description, input_description?, output_description?, difficulty, time_limit_ms, memory_limit_mb, samples[]} |
...
2. **验题时效**：题目内容（题面等）变更不影响验题有效性；**测试点 / 样例在最近一次验题通过后发生变更，则须重新走验题流程才能发布**——判定依据为 `MAX(test_cases.updated_at) > problems.verified_at`，发布接口违反时返回 3002；`scope=mine` 列表与详情分别以 `needs_reverification` 字段透出。

...
- **题目管理** `/admin/problems`（admin / tutor）：管理工作台。状态标签页（全部 / 草稿 / 已发布 / 已归档）+ 搜索；表格列含状态与验题状态标签（需重验时警示），操作列按状态给动作 —— 草稿：编辑；已发布：详情 / 编辑 / 归档；已归档：详情。行点击：草稿 → 编辑向导，已发布 → 详情；已归档行为只读，行点击不跳转（查看走操作列「详情」，避免被带出管理后台）。发布统一在编辑向导第三步完成，列表不提供独立发布入口。
